...
authentication system beyond a single admin bearer token. Entries below are the ones that apply — STRIDE
...
| 2 | Spoofing | Someone steals/guesses `ADMIN_TOKEN` and reads submission PII | Token is a 64-char cryptographically random hex string (`secrets.token_hex(32)`), transmitted only over HTTPS, never logged | Mitigated |
...
| 6 | Information Disclosure | The three admin endpoints leak visitor PII to unauthenticated requests | **This was Phase 0 Finding 1** — now requires `Authorization: Bearer <ADMIN_TOKEN>`, fails closed (503) if unconfigured | Mitigated, tested |
...
| 11 | Denial of Service | Admin token leaked, used to hammer the read endpoints | Separate, higher rate limit (120/15min) still applies even with a valid token | Mitigated |
| 12 | Elevation of Privilege | N/A — no privilege hierarchy exists (visitor vs. the one admin token, nothing in between) | — | Not applicable |
| 13 | Elevation of Privilege | CORS misconfigured, allowing a malicious site to make authenticated-looking requests on a visitor's behalf | `FRONTEND_URL` scopes CORS to the real frontend origin in production; note this doesn't protect the admin endpoints since they don't use cookies/session — the bearer token isn't attached automatically by a browser, so CSRF-style attacks don't apply to them | Mitigated by design (token auth, not cookie auth) |
